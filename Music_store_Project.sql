...
select billing_country as Country,count(*) from invoice
...
select c.first_name,c.last_name,sum(total) as total_spent from customer c
...
select distinct email,first_name,last_name from customer c join invoice i 
on c.customer_id=i.customer_id join invoice_line il on
i.invoice_id=il.invoice_id where track_id in
...
select a.name,count(a.artist_id) as Total_track_count from artist a
join
